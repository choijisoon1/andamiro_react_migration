...
Pinia는 최종 정리에서 프로젝트에서 완전히 제거했다. 기존 Pinia 저장소의 모든 코드를 Zustand로만 1:1 변환한 것이 아니라, Pinia가 담당하던 로직을 데이터 성격에 따라 Zustand와 TanStack Query로 나눠서 이관했다.
...
화면 이관과 소스 대조가 끝난 뒤 아래 기존 저장소를 제거했다. 현재 실행 구조에는 Zustand 저장소와 TanStack Query 계층만 남아 있다.
...
### 2026-07-24: React 정리와 통합 테스트 전 안정화

- `c0c36f3` Vue·Pinia 실행 코드 제거와 React 이관 최종 정리
- `2a6096e` 초대 미리보기의 사용자 인증을 확인하고 비밀번호 검증 전 공개 필드를 `id`, `title`로 제한
- `d6327c6` 하드코딩 외부 워크플로 주소를 제거하고 명시적 환경변수 방식으로 변경
- 통합 테스트 전 재점검 결과와 이후 수정 이력은 `docs/code-review.md`에서 누적 관리
...
| `src/router/index.js` | `src/router/reactRouter.jsx` | React Router와 인증·회원가입 보호 경로로 이관 완료 |
| `src/views/SplashView.vue` | `src/App.jsx`의 `SplashScreen` 및 `reactRouter.jsx`의 `LandingRoute` | 스플래시와 진입 분기 기능을 합쳐서 이관 완료 |

기존 `src/router/index.js`는 모든 React 경로가 연결된 뒤 제거했다.
...
| `src/stores/exchange.js` | `src/api/exchangeApi.js` + `src/queries/exchangeQueries.js` | TanStack Query | 교환일기 서버 로직 분리 완료 |
| `src/stores/counter.js` | 대상 없음 | 제거 | 예제 저장소라 최종 정리에서 삭제 |

표의 기존 Pinia 파일은 모두 삭제됐다. 현재 실행 구조에서 사용하는 것은 위 표의 Zustand 저장소와 TanStack Query 계층이다.
...
| `src/views/exchange/RoomView.vue` | 대상 없음 | 없음 | 실제 이동 경로와 기능이 없는 임시 화면으로 확인 후 삭제 |
...
| `src/views/my/ChatListView.vue` | 대상 없음 | 없음 | 실제 라우트와 기능이 없는 임시 화면으로 확인 후 삭제 |
...
| `src/components/layout/FooterDouble.vue` | 대상 없음 | 실제 사용처가 없어 삭제 |
| `src/components/layout/AppLayout.vue` | 대상 없음 | 실제 사용처가 없어 삭제 |
| `src/components/HelloWorld.vue` | 대상 없음 | 예제 파일이라 삭제 |
...
| `src/composables/usePushSubscription.js` | 푸시 구독 로직, React 마이페이지에서 사용 |
| `src/sw.js` | Service Worker |
| `api/chat.js` | Vercel 서버리스 AI Proxy |
| `supabase/functions/**` | Supabase Edge Functions |
| `supabase/migrations/**` | 데이터베이스 마이그레이션 |
| `public/assets/**` | 기존 이미지·아이콘·영상 |
| `src/assets/scss/**` | 기존 디자인을 유지하는 React 공통 스타일 |

이 매핑표는 삭제된 Vue·Pinia 원본이 어느 React 파일로 이관됐는지 확인하는 완료 기록으로 유지한다.

## 6. 현재 React 이관 범위

실제 사용자 라우트 기준 총 19개 동작 지점이 모두 React 구현에 연결됐다. 화면 코드 연결과 Vue·Pinia 제거는 완료됐으며, 실제 계정·권한·외부 서비스까지 포함한 통합 검증은 남아 있다.
...
| `/exchange/room` | 기능 없는 Vue 임시 화면 | React 라우트와 Vue 파일 제거 |
...
`FooterDouble.vue`처럼 React 실행 경로에서 사용하지 않는 Vue 컴포넌트는 새로 이관하지 않고 제거했다. 저장된 일기 상세의 Gauge는 기존 240×168 크기와 1.2초 애니메이션을 유지하는 `SvgGauge.jsx`를 재사용했다.

`RoomView.vue`, `HelloWorld.vue`, `counter.js`, `ChatListView.vue`는 실제 사용처가 없는 임시·예제 파일로 확인하고 제거했다.
...
### 단계 E — Vue·Pinia 제거 (완료)

1. [x] `MigrationPlaceholder.jsx` 제거
2. [x] 사용하지 않는 `.vue` 파일과 Vue 진입점·라우터 제거
3. [x] 기존 Pinia 저장소 제거
4. [x] Vue 전용 ESLint 설정 제거
5. [x] 아래 패키지 제거
...
6. [x] Vue·Pinia import가 0개인지 전체 검색
7. [x] lockfile 갱신 후 `npm ci`, `npm run lint:check`, `npm run build` 재검증
...
통합 테스트 전 전체 코드 재점검에서 확인된 Vue 원본 승계 결함과 React 안정화
항목은 [코드 리뷰 — 결함 보고서](./code-review.md)의
`통합 테스트 전 재점검`에서 수정 순서, 변경 이유, 검증 및 배포 상태를
누적 관리한다. 첫 번째 항목인 초대 미리보기 본문·이미지 노출은 소스 수정이
완료됐으며, 두 번째 항목인 하드코딩 외부 워크플로 주소도 명시적 환경변수가
있을 때만 실행하도록 수정했다. 세 번째 항목인 개인 일기 날짜는 현지 달력
기준 공통 함수로 통일했다. 네 번째 항목은 프로필 행이 없는 정상 응답과 서버
조회 오류를 구분해 기존 사용자가 일시적인 오류 때문에 신규 가입 화면으로
이동하지 않도록 수정했다. 다섯 번째 항목은 TanStack Query의 서버 조회 오류를
실제 빈 데이터와 분리하고 기존 화면 스타일을 재사용한 재시도 UI를 연결했다.
앞의 두 항목은 Supabase Edge Function 재배포 후 실제 응답과 외부 요청 여부
검증이 남아 있고, 날짜·인증·조회 오류 분기는 브라우저 통합 흐름에서 확인해야
한다.
...
`npm audit fix`로 ECharts 6.1.0, Vite 8.1.5와 적용 가능한 하위 패키지 보안
패치를 반영했다. 남은 3건(낮음 2, 높음 1)은 `face-api.js`가 사용하는 오래된
TensorFlow·`node-fetch` 계층이며, 자동 수정은 `face-api.js`를 0.20.0으로
강제 변경한다. 카메라 분석 회귀 위험 때문에 `--force`는 적용하지 않고 향후
얼굴 분석 라이브러리 교체 또는 별도 검증 항목으로 남긴다.

## 11. 최종 완료 체크리스트

- [x] 모든 사용자 라우트에서 `MigrationPlaceholder` 제거
- [ ] 기존 Vue와 동일한 핵심 기능 및 화면 유지
- [x] Supabase 서버 상태를 API 모듈 + TanStack Query로 관리
- [x] 필요한 전역 클라이언트 상태를 Zustand로 관리
- [x] 런타임에서 `.vue`, Pinia, Vue Router를 사용하지 않음
- [x] Vue 전용 패키지와 설정 제거
- [x] `npm ci` 후 `npm run lint:check` 성공
- [x] `npm run build` 성공
- [ ] 로그인부터 일기 저장·조회까지 전체 흐름 검증
- [ ] 교환일기 초대·댓글·권한 검증
- [ ] PWA·카메라·마이크·푸시의 허용/거부 흐름 검증
- [ ] Vercel·Supabase 환경변수와 배포 확인
- [x] README와 개발 가이드를 React 기준으로 갱신

## 12. 바로 다음 작업

화면 코드 연결 19/19와 Vue·Pinia 제거가 완료됐다. 다음 작업은 React 전체 흐름을 실제 브라우저와 배포 환경에서 회귀 검증하는 것이다.
...
`MigrationPlaceholder.jsx`, Vue 원본 화면, Vue 진입점·라우터, Pinia 저장소와 Vue 전용 패키지는 최종 정리에서 제거했다. 삭제 이력과 원본은 Git 기록에서 확인하고 복구할 수 있다.
